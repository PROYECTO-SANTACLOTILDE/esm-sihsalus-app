...
import { Finding39Design1 } from '../../../ui/teeths/odontology-design';
import './SpaceBetweenStyles.css';

const SpacingBetweenFinding39 = ({ id }) => {
  ...
  const legend = intermediateSpaceOnTheCanvasOfFinding39?.find((item) => item.id === id);
        ...
        <>
          {findings.map((finding) => (
            <React.Fragment key={finding.uniqueId}>
              {finding.dynamicDesign === 1 && finding.color && <Finding39Design1 strokeColor={finding.color.name} />}
            </React.Fragment>
          ))}
        </>
      )}
