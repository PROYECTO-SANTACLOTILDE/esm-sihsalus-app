...
export const useAttentions = (
  patientUuid: string,
): { prenatalEncounters: ObsEncounter[]; error: any; isValidating: boolean } => {
  const atencionPrenatal = 'Atención prenatal';
  const attentionssUrl = useMemo(() => {
    return `${restBaseUrl}/encounter?patient=${patientUuid}&encounterType=${atencionPrenatal}`;
  }, [patientUuid]);

  const { data, error, isValidating } = useSWR<EncounterResponse>(patientUuid ? attentionssUrl : null, async (url) => {
  ...
  const prenatalEncounters = useMemo(() => {
    ...
    return detailedEncounters.filter((encounter) => encounter?.form?.display === 'Atención prenatal');
  }, [detailedEncounters]);
