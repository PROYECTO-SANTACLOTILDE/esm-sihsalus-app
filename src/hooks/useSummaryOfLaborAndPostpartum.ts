...
export function useSummaryOfLaborAndPostpartum(patientUuid: string) {
  ...
  const mostRecentEncounterId = useMemo(() => {
    ...
    const laborPostpartumEncounters = encounters.filter(
      (encounter) => encounter?.form?.display === 'OBST-004-RESUMEN DE PARTO Y PUERPERIO'
    );

    if (!laborPostpartumEncounters.length) return null;

    return laborPostpartumEncounters.sort((a, b) =>
